...
So I made an account and started messing around with the actual app. I observed that expense submissions go out as a JSON payload over an API, all three fields get inserted as strings, and inserts are handled entirely server-side. No obvious injection surface there either.
